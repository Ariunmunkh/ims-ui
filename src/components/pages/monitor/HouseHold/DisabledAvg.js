...
export default function DisabledAvg() {
    ...
    const getgendercount = async (tstatus, tgender) => {
        ...
        await api
            .get(`/api/record/report/get_disabled_avg?status=${tstatus}&gender=${tgender}`)
            .then((res) => {
                if (res?.status === 200 && res?.data?.rettype === 0) {
    ...
    const getgendercountdistrict = async (tstatus, tgender, tdistrict) => {
        ...
        await api
            .get(`/api/record/report/get_disabled_avg_district?status=${tstatus}&gender=${tgender}&districtid=${tdistrict ?? 0}`)
            .then((res) => {
                if (res?.status === 200 && res?.data?.rettype === 0) {
