...
export default function Participant() {
    ...
    const getgendercount = async (tstatus) => {
        ...
        await api
            .get(`/api/record/report/get_participant_gender_count?status=${tstatus}`)
            .then((res) => {
                if (res?.status === 200 && res?.data?.rettype === 0) {
    ...
    const getgendercountdistrict = async (tstatus, tdistrict) => {
        ...
        await api
            .get(`/api/record/report/get_participant_gender_count_district?status=${tstatus}&districtid=${tdistrict ?? 0}`)
            .then((res) => {
                if (res?.status === 200 && res?.data?.rettype === 0) {
