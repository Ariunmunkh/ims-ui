...
export default function HouseHold() {
    ...
    const getgendercount = async (tstatus) => {
        ...
        await api
            .get(`/api/record/report/get_household_gender_count?status=${tstatus}`)
            .then((res) => {
                if (res?.status === 200 && res?.data?.rettype === 0) {
    ...
    const getgendercountdistrict = async (tstatus, tdistrict) => {
        ...
        await api
            .get(`/api/record/report/get_household_gender_count_district?status=${tstatus}&districtid=${tdistrict ?? 0}`)
            .then((res) => {
                if (res?.status === 200 && res?.data?.rettype === 0) {
                ...
                <Row gutter={16} style={{ marginTop: 16, marginBottom: 16 }}>
                    <Col span={24}>
                        ...
                        >
                            ...
                            <Tooltip />
                            <Legend />
                            <Bar dataKey="count" stackId="a" name={"Нийт"} fill="#8884d8" />
                        </BarChart>
                    </Col>
                </Row>
                <Row gutter={16} style={{ marginTop: 16, marginBottom: 16 }}>
