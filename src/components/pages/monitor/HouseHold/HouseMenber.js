...
export default function HouseMenber() {
    ...
    const getgendercount = async (tstatus, tgender) => {
        ...
        await api
            .get(`/api/record/report/get_housemenber_avg?status=${tstatus}&gender=${tgender}`)
            .then((res) => {
                if (res?.status === 200 && res?.data?.rettype === 0) {
    ...
    const getgendercountdistrict = async (tstatus, tgender, tdistrict) => {
        ...
        await api
            .get(`/api/record/report/get_housemenber_avg_district?status=${tstatus}&gender=${tgender}&districtid=${tdistrict ?? 0}`)
            .then((res) => {
                if (res?.status === 200 && res?.data?.rettype === 0) {
                ...
                <Row gutter={16} style={{ marginTop: 16, marginBottom: 16 }}>
                    <Col span={24}>
                        ...
                        >
                            ...
                            <Tooltip />
                            <Legend />
                            <Bar dataKey="avg" stackId="a" name={"дундаж"} fill="#8884d8" />
                        </BarChart>
                    </Col>
                </Row>
                <Row gutter={16} style={{ marginTop: 16, marginBottom: 16 }}>
