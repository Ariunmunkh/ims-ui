...
import { Table, Modal, Drawer, Form, Space, Button, Input, Tabs } from "antd";
...
export default function Hutulbur() {
  ...
  const [formtitle] = useState("Хөтөлбөр");
  const [formtitle2] = useState("Хөтөлбөрийн индикатор");
  const [formtitle3] = useState("Насны ангилал");
  ...
  const onDelete = async () => {
    await api
      .delete(
        `/api/record/base/delete_dropdown_item?id=${formdata.getFieldValue(
          "id"
        )}&type=${formtype}`
      )
      .then((res) => {
  ...
  const getFormData = async (id) => {
    await api
      .get(`/api/record/base/get_dropdown_item?id=${id}&type=${formtype}`)
      .then((res) => {
        if (res?.status === 200 && res?.data?.rettype === 0) {
    ...
    <div>
      ...
      >
        <Form form={formdata} labelCol={{ span: 8 }} wrapperCol={{ span: 14 }}>
          ...
          <Form.Item name="name" label="Хөтөлбөрийн нэр">
            <Input />
          </Form.Item>
        </Form>
      </Drawer>
