...
export default function Volunteer() {
    ...
    const fetchData = useCallback(async () => {
        ...
        await api
            .get(`/api/Volunteer/get_Volunteer_image?id=${volunteerid ?? userinfo.volunteerid}`)
            .then((res) => {
                if (res?.status === 200 && res?.data?.rettype === 0) {
                    if (res?.data?.retdata)
                        setPostImage(res?.data?.retdata);
                }
            });
    ...
    const handleFileUpload = async (e) => {
        ...
        await api.post(`/api/Volunteer/set_Volunteer_image`, { volunteerid: volunteerid ?? userinfo.volunteerid, image: base64 }).then((res) => {
            if (res?.status === 200 && res?.data?.rettype === 0) {
