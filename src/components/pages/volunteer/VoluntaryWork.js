...
export default function VoluntaryWork() {
  ...
  const fetchData = useCallback(() => {
    ...
    api
      .get(
        `/api/Volunteer/get_VolunteerVoluntaryWork_list?id=${
  ...
  const onFinish = async (values) => {
    ...
    fdata.voluntaryworkdate = fdata.voluntaryworkdate.format(
      "YYYY.MM.DD HH:mm:ss"
    );
    fdata.status = 0;
    await api
    ...
    await api
      ...
      .then((res) => {
        if (res?.status === 200 && res?.data?.rettype === 0) {
          ...
          fdata.voluntaryworkdate = dayjs(
            fdata.voluntaryworkdate,
            "YYYY.MM.DD HH:mm:ss"
          );
