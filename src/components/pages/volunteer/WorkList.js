...
import { Col, Row, Table, Button, Input, Space, Tag } from "antd";
                    ...
                    if (res?.status === 200 && res?.data?.rettype === 0) {
                        if (userinfo.committeeid) {
                            const filteredData = res.data.retdata.filter(
                                (item) => item.status === "0" || item.status === 0 || item.status == null
                            );
    ...
    const gridcolumns = [
        {
            ...
            key: "action",
            render: (text, record) => (
                <Button type="link" onClick={() => navigate(`/volunteer/${record.volunteerid}`)}>
